...
[Script]
http-request ^https:\/\/api\.wetalkapp\.com\/app\/queryBalanceAndBonus script-path=WeTalk_Loon.js,tag=WeTalk抓取账号,requires-body=false,timeout=10,enable=true
cron "20 8,20 * * *" script-path=WeTalk_Loon.js,tag=WeTalk签到,timeout=300,enable=true
...
const SCRIPT_VERSION = '2026.06.06-loon.4';
...
function httpGet(options) {
  return new Promise(function(resolve, reject) {
    ...
    if (typeof $httpClient !== 'undefined' && $httpClient.get) {
      ...
      $httpClient.get(req, function(err, response, data) {
        if (err) {
          settleReject({ error: String(err) });
          return;
        }
        settleResolve({
